...
phasecorr::Image to_phasecorr_image(const Image &image, std::size_t parallelism = 1) {
  phasecorr::Image gray(image.height, image.width);
  const int height = image.height;
  const int width = image.width;
  parallel_for_no_progress(static_cast<std::size_t>(height), parallelism, [&](std::size_t y) {
    for (int x = 0; x < width; ++x) {
      const std::size_t idx =
          (y * static_cast<std::size_t>(width) + static_cast<std::size_t>(x)) * 3U;
      const float r = static_cast<float>(image.pixels[idx]);
      const float g = static_cast<float>(image.pixels[idx + 1]);
      const float b = static_cast<float>(image.pixels[idx + 2]);
      gray(static_cast<int>(y), x) = 0.299f * r + 0.587f * g + 0.114f * b;
    }
  });
  return gray;
}
...
phasecorr::Image rotate_phasecorr_image(const phasecorr::Image &image, double rotation_degrees, std::size_t parallelism = 1) {
  ...
  parallel_for_no_progress(static_cast<std::size_t>(image.rows), parallelism, [&](std::size_t row) {
...
phasecorr::Image downsample_phasecorr_image_half(const phasecorr::Image &image, std::size_t parallelism = 1) {
  ...
  parallel_for_no_progress(static_cast<std::size_t>(downsampled_rows), parallelism, [&](std::size_t row) {
    const int y = static_cast<int>(row);
...
void prepare_alignment_cache(const std::vector<InputEntry> &entries, const Fs::path &cache_dir, std::size_t parallelism) {
  std::error_code ec;
  ...
  const std::size_t worker_limit = std::max<std::size_t>(1, std::min(parallelism, entries.size()));
  ...
  auto finish_one = [&]() {
    ...
    pending.pop_front();
    ++completed;
    print_progress("preparing alignment cache", completed, entries.size());
  };

  for (std::size_t i = 0; i < entries.size(); ++i) {
    Image image = load_image_for_alignment(entries[i].path);
    if (pending.size() >= worker_limit) {
      finish_one();
    }
    pending.push_back(std::async(std::launch::async, [i, &cache_dir, image = std::move(image)]() mutable {
      // parallelism=1: we're already running multiple async tasks in parallel
      const phasecorr::Image gray = to_phasecorr_image(image, 1);
...
Transform estimate_transform_for_angles(const phasecorr::Image &reference,
  ...
  std::vector<double> peaks(n, -std::numeric_limits<double>::infinity());
  std::vector<double> dxs(n, 0.0);
  std::vector<double> dys(n, 0.0);

  parallel_for_no_progress(n, parallelism, [&](std::size_t i) {
    // parallelism=1 for rotate since we're already in a parallel context
    ...
    peaks[i] = result.peak;
    dxs[i] = result.shift.x();
    dys[i] = result.shift.y();
  });
  ...
  t.dx = dxs[best];
  t.dy = dys[best];
  t.rotation_degrees = angles[best];
  return t;
}
...
AlignmentData compute_alignment_from_entries(const std::vector<InputEntry> &entries,
  ...
  if (worker_count == 1) {
    ...
    phasecorr::Image previous_gray_half = downsample_phasecorr_image_half(previous_gray, parallelism);
    for (std::size_t index = 1; index < entries.size(); ++index) {
      phasecorr::Image current_gray = read_gray_image_cache(alignment_gray_cache_path_for(alignment_cache_dir, index));
      phasecorr::Image current_gray_half = downsample_phasecorr_image_half(current_gray, parallelism);
  ...
  auto worker = [&](std::size_t worker_index) {
    ...
    try {
      ...
      for (std::size_t index = start_index; index <= end_index; ++index) {
        ...
        const Transform delta = scale_transform(
            estimate_transform_phase_corr(previous_gray, current_gray, previous_gray_half, current_gray_half, 1));
        alignment.relative_transforms[index] = delta;
        print_alignment_progress(operation, index, task_count, delta);
        previous_gray = std::move(current_gray);
        previous_gray_half = std::move(current_gray_half);
